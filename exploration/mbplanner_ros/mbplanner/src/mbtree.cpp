...
void MBTree::sampleAccelUsingLaser(Eigen::Vector3d& accl, double theta, StateNode* front_node){
  

  // ROS_INFO("current time = %f", ros::Time::now().toSec());
  // ROS_INFO("current time = %f", laser_data.header.stamp.toSec());
  // double time_diff = fabs(ros::Time::now().toSec()-laser_data.header.stamp.toSec());
  ...
  //       "/base_link",
  //       laser_data.header.stamp + ros::Duration().fromSec(laser_data.ranges.size()*laser_data.time_increment),
  //       ros::Duration(1)) || laser_data.ranges.size() == 0 ){
  //       ROS_INFO("no such transform from lidar to base link, mbtree");
  //      ROS_INFO("Laser points are lagging .. need to make buffer");
  //    double mag = (*distribution_y_)(gen_);  
  //     ax = mag * cos(theta);
  //     ay = mag * sin(theta);     
  // }else{  
...
int MBTree::buildTree(bool prev_point_plan, bool use_current_state, bool use_given_vel) {
 ...
 // StateNode Tree que initialization 
...
void MBTree::setLaser(const sensor_msgs::LaserScanConstPtr& msg) { 
  this->laser_data = *msg;
}

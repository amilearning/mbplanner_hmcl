...
int MBTree::buildTree(bool prev_point_plan, bool use_current_state, bool use_given_vel) {
  ...
  */
  ...
  current_sample_steps = 3;
  StateNode* prev_d_state = new StateNode;
 // StateNode Tree que initialization 
  std::queue<StateNode*> q;
  // set root tree 
  q.push(tree_root_);
  // for( int iter=0; iter < mb_params_.iterations; ++iter){
    for( int iter=0; iter < 2; ++iter){
    int n = q.size();
    for(int tl = 0; tl < current_sample_steps+1; tl++){
      for (int c = 0; c < current_sample_steps; c++) {
          double diff_theta = M_PI/4*(c-1);
          double theta = q.front()->yaw - diff_theta;
          truncateYaw(theta);
          double mag = -3*(iter-2)+2;          
          double ax = mag * cos(theta);
          double ay = mag * sin(theta);
          double az = 0.0;          
          ...
          std::vector<StateNode*> temp_inter_state_tree_;  

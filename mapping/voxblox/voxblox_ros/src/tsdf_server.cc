...
void TsdfServer::processPointCloudMessageAndInsert(
    ...
    const Transformation& T_G_C, const bool is_freespace_pointcloud, bool lidar_source) {
...
////////////////////////////////////////////////////////////////////   
  ...
  if(!lidar_source){
      if (color_pointcloud) {
        ...
        sor.setLeafSize (0.3f, 0.3f, 0.3f); //leaf size
  ...
  }else{  //if points are from Lidar, do tranformation and process 
    pcl::PointCloud<pcl::PointXYZ> pointcloud_pcl;
    ...
    pointcloud_pcl = *lidar_cloud_tmp;    
     convertPointcloud(pointcloud_pcl, color_map_, &points_C, &colors);
  }
  ptcloud_timer.Stop();
...
void TsdfServer::lidar_inserPointcloud(
    const sensor_msgs::LaserScan::Ptr& laser_pointcloud) {    
  lidar_sub_done =true;
...
void TsdfServer::insertFreespacePointcloud(
  ...
  ROS_INFO("integrate free space point cloud");
  Transformation T_G_C;
  sensor_msgs::PointCloud2::Ptr pointcloud_msg;
...
Eigen::Affine3d TsdfServer::transformTFToAffine3d(const tf::Transform &t) {
    Eigen::Affine3d e;
